...
var Sendgrid = function(api_user, api_key, options) {
  ...
  var stats = function(email, callback) {
    // set correct stats enpoint 
    uriParts.endpoint = "/api/stats.get.json"
    this.options.uri = uriParts.protocol + "://" + uriParts.host + (uriParts.port ? ":" + uriParts.port : "") + uriParts.endpoint;
    var callback    = callback || function() { };
    if (email.constructor !== Email) {
      email = new Email(email);
    }
